...
/**
 * 日本語のタグ名マッピングを取得
 * @param {string} key - タグのキー
 * @returns {string} - 日本語のタグ名
 */
export const getJapaneseTagName = key => {
  const japaneseTagNames = {
    Make: 'メーカー',
    Model: '機種',
    DateTime: '撮影日時',
    ExposureTime: '露出時間',
  ...
  return japaneseTagNames[key] || key;
};

/**
 * デフォルトで選択するEXIFタグのリスト
 */
